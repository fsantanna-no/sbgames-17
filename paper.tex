...
\caption{Continuation mechanism and state machine for advancing pages in the \emph{Story screen}.
\label{fig.story}
}
\end{figure}
...
implements the method \code{next\_text}, which is a callback for clicks in the
button \code{>>>}.
%
The variable \code{pages} (ln. 4--5, 24--26) is a vector holding each page, but
...
(ln. 6--15) instead of explicit state variables for the continuation mechanism
and state machine.
...
This way, we do not need a variable (such as \code{displayed} in C++) to switch 
