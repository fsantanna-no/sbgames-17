...
%
\CEU uses the direct style techniques in 5 cases involving screen transitions:
...
\subsubsection{Case Study: Global Keys and the Options Menu}
...
First-class internal events of \CEU provide some advantages in comparison to 
Boost signals:

\begin{itemize}
\item They use the same convenient syntax of external events (e.g.,
      \code{emit}, \code{await}, \code{every}, etc.).
\item They never create infinite dependency loops.
\item They do not require explicit unbinding.
\end{itemize}

The implementation in \CEU uses 39 events for internal communication defined in
23 files with over 200 invocations of \code{emit} and \code{await} spread in
over 50 files.
