...
analysis of the proposed solutions in \CEU in comparison to the original
...
code in direct and sequential style in order to recover from the inversion of
control imposed by the observer pattern~\cite{rp.deprecating}.
%
...
dependency and explicit ordering between statements (or groups of statements).
...
Although our analysis is mostly qualitative, the rows in Figure~\ref{tab.tree}
...
During the rewriting process, we have identified four abstract control-flow
patterns which likely apply to other games as well:
...
The circles represent the state of the variables in the class, and the arrows 
...
5 of them, enumerated in Figure~\ref{tab.actions}, implement at least one state
machine and are considerable smaller in \CEU in terms of LoCs (after
...
the story pages.
For ending stories, the game also shows a \emph{Credits screen} before
...
during gameplay (e.g., \emph{walking} to \emph{falling} and back to
\emph{walking}).
...
\item The \code{World} class manages and dispatches events to all game
      entities with a common interface \code{WorldObj}, such as all pingus
      together and traps (step 4).
...
and also respects the program static hierarchy.
...
%
If the programmer disregards the call to \code{remove}, a dead pingu would
remain in the collection and still update on every frame (ln. 11).
...
%
Entities with dynamic lifespan in C++ always require explicit matching
...
gameplay \& worldmap objects, and also UI widgets.
...
patterns as implementation techniques to achieve the desired higher-level
...
%
In the end, we rewrote about 1/4 of the whole codebase (9.186 from 39.362 lines
...
continuation variables spread across multiple classes (e.g., stacks and boolean
flags).
